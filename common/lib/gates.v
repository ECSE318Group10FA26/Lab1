...
module and_n #(
    ...
    buf1 #(N) seed (.a({N{1'b1}}), .y(chain[N-1:0]));
    ...
    buf1 #(N) out_buf (.a(chain[I*N +: N]), .y(y));
endmodule
...
module or_n #(
    ...
    buf1 #(N) seed (.a({N{1'b0}}), .y(chain[N-1:0]));
...
module and_2n #(
    ...
    generate
        if (I == 1) begin : g_base1
            buf1 #(N) g_buf (.a(d), .y(y));
        end else if (I == 2) begin : g_base2
            and2 #(N) g_and (.a(d[N-1:0]), .b(d[2*N-1:N]), .y(y));
        end else begin : g_rec
            localparam L = 1 << $clog2(I) - 1;
            wire [N-1:0] lo, hi;
            and_2n #(.N(N), .I(L))   m_lo (.d(d[0    +: L*N]),    .y(lo));
            and_2n #(.N(N), .I(I-L)) m_hi (.d(d[L*N +: (I-L)*N]), .y(hi));
            and2   #(.N(N))          g_and (.a(lo), .b(hi), .y(y));
        end
    endgenerate
endmodule
...
module or_2n #(
    ...
    generate
        if (I == 1) begin : g_base1
            ...
            or2 #(N) g_or (.a(d[N-1:0]), .b(d[2*N-1:N]), .y(y));
        end else begin : g_rec
            localparam L = 1 << $clog2(I) - 1;
            wire [N-1:0] lo, hi;
            or_2n #(.N(N), .I(L))   m_lo (.d(d[0    +: L*N]),    .y(lo));
            or_2n #(.N(N), .I(I-L)) m_hi (.d(d[L*N +: (I-L)*N]), .y(hi));
            or2   #(.N(N))          g_or (.a(lo), .b(hi), .y(y));
        end
    endgenerate
endmodule
...
module xor_2n #(
    ...
    generate
        if (I == 1) begin : g_base1
            ...
            xor2 #(N) g_xor (.a(d[N-1:0]), .b(d[2*N-1:N]), .y(y));
        end else begin : g_rec
            localparam L = 1 << $clog2(I) - 1;
            wire [N-1:0] lo, hi;
            xor_2n #(.N(N), .I(L))   m_lo (.d(d[0    +: L*N]),    .y(lo));
            xor_2n #(.N(N), .I(I-L)) m_hi (.d(d[L*N +: (I-L)*N]), .y(hi));
            xor2   #(.N(N))          g_xor (.a(lo), .b(hi), .y(y));
        end
    endgenerate
endmodule
