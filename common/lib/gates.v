...
//   and_2n #(N,I): y = AND of 2**I packed words (recursive binary tree)
...
// 2**I-input, N-bit-wide AND, recursive binary tree
...
module and_2n #(
    ...
    generate
        if (I == 1) begin : g_base
            ...
            localparam H = 1 << (I-1);  // input words per half
            wire [N-1:0] lo, hi;
            and_2n #(.N(N), .I(I-1)) m_lo (.d(d[0    +: H*N]), .y(lo));
            and_2n #(.N(N), .I(I-1)) m_hi (.d(d[H*N +: H*N]), .y(hi));
...
module or_2n #(
    ...
    generate
        if (I == 1) begin : g_base
            ...
            localparam H = 1 << (I-1);  // input words per half
            wire [N-1:0] lo, hi;
            or_2n #(.N(N), .I(I-1)) m_lo (.d(d[0    +: H*N]), .y(lo));
            or_2n #(.N(N), .I(I-1)) m_hi (.d(d[H*N +: H*N]), .y(hi));
