...
module divider_tb;
  ...
  task automatic run_test;
    ...
    begin
      ...
      if (done !== 1'b1) begin
        $display("FAIL: %0d/%0d  TIMEOUT waiting for done (divider not implemented yet?)", dvd,
                 dvr);
        errors = errors + 1;
      end else if (quotient === exp_q && remainder === exp_r) begin
        $display("PASS: %0d/%0d  quotient = %0d  remainder = %0d", dvd, dvr, quotient, remainder);
      end else begin
        $display({"FAIL: %0d/%0d  got quotient = %0d remainder = %0d, ",
                  "expected quotient = %0d remainder = %0d"}, dvd, dvr, quotient, remainder, exp_q,
                   exp_r);
        errors = errors + 1;
      end
    end
